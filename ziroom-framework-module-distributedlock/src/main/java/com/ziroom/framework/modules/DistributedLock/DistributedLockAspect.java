...
import com.ziroom.qms.common.enums.QmsCode;
import com.ziroom.qms.common.exception.QmsException;
...
public class DistributedLockAspect {
    ...
    @Pointcut("@annotation(com.ziroom.qms.web.annotation.DistributedLock)")
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (index == ArrayUtil.INDEX_NOT_FOUND) {
            throw new QmsException(QmsCode.TRY_LOCK_FAIL.getCode(), "DistributedLockAspect：获取锁名称失败");
        }
        ...
        if (StrUtil.isBlank(lockName)) {
            throw new QmsException(QmsCode.TRY_LOCK_FAIL.getCode(), "DistributedLockAspect：锁名称为空");
        }

        log.info("加分布式锁，lockName = {}", lockName);
        RLock lock = redisson.getLock(lockName);
        if (!lock.tryLock()) {
            throw new QmsException(QmsCode.TRY_LOCK_FAIL.getCode(), "资源正在使用，请稍后再试");
        }
        try {
