...
public class DefaultServerProvider implements ServerProvider {
  ...
  private void initRemote() {
    // 1. Try to get m_remote from JVM system property
    m_remote = System.getProperty("remote");
    if (!Utils.isBlank(m_remote)) {
      m_remote = m_remote.trim();
      logger.info("Remote is set to [{}] by JVM system property 'remote'.", m_remote);
      return;
    }

    // 2. Try to get m_remote from OS environment variable
    m_remote = System.getenv("REMOTE");
    if (!Utils.isBlank(m_remote)) {
      m_remote = m_remote.trim();
      logger.info("Remote is set to [{}] by OS env variable 'REMOTE'.", m_remote);
      return;
    }

    // 3. Try to get m_remote from file "server.properties"
    m_remote = m_serverProperties.getProperty("remote");
    if (!Utils.isBlank(m_remote)) {
      m_remote = m_remote.trim();
      logger.info("Remote is set to [{}] by property 'remote' in server.properties.", m_remote);
      return;
    }

    // 4. Set m_remote to 1.
    m_remote = "true";
    logger.info("Remote is set to null. Because it is not available in either (1) JVM system property 'remote', (2) OS env variable 'REMOTE' nor (3) property 'm_remote' from the properties InputStream.");
  }
